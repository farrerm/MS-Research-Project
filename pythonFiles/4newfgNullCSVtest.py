...
def issueSearch(repoName, commitID):
	...
	#outPut != '[\n\n]\n':
		str1 = 'curl'
		str2 = "-H"
		str3 = 'Authorization: token 4fcad2bb2b13f28be1ec7027e8c8ee6d70b208d0'
		str4 = 'https://api.github.com/repos/'
...
with open('/Users/matt/eclipse-workspace/nuCSV.csv', newline='') as csvfile:
	...
	with open('/Users/matt/eclipse-workspace/5myNulls.csv', 'w', newline='') as nullsCSV:
		numMethods = 0
		numFields = 0
		numParams = 0
		numLVs = 0
...
with open('/Users/matt/eclipse-workspace/5myNulls.csv', newline='') as nullsCSV2:
...
with open('/Users/matt/eclipse-workspace/12fgTestNullAnnotations.csv', 'w', newline = '') as outcsv:
	writer = csv.writer(outcsv)
	writer.writerow(['Repo', 'Path', 'Node Type', 'Class', 'FQN', 'Parent FQN', 'Annotation', 'Commit ID', 'Comitter', 'Date', 'Commit Message', 'Pulls Merged',
		'Issues Referenced', 'Package'])
	outcsv.flush()
		...
		for commit in RepositoryMining(repoName).traverse_commits():
			...
			if commit.in_main_branch:
				...
				#we don't care if it contains path any more
					...
					#rowAsList.append(row['Package'])
					#rowAsList.append(commitFileName)
							...
							if contains(rowAsList, bigList):
								...
								print('found')
									##write a row to csv for that commit/annotation
								wrapper = textwrap.TextWrapper(width=50, replace_whitespace=True)
								listToWrite = []
								listToWrite.append(key)
								curPath = getCurPath(rowAsList, bigList)
							
								listToWrite.append(curPath)
								annotationType = getAnnotationType(rowAsList, bigList)
								listToWrite.append(annotationType)
							#listToWrite.append(row['Path'])
							#listToWrite.append(row['Package'])
							#listToWrite.append(commitFileName)
								listToWrite.append(row['Class'])
								listToWrite.append(row['FQN Parent'])
								listToWrite.append(row['FQN'])
								listToWrite.append(row['Annotation'])

								listToWrite.append(str(commit.hash))
								listToWrite.append(str(commit.author.name))
								listToWrite.append(commit.author_date)
									##add committer ID and date
								#underline = b'\xcd\x9f'.decode('utf-8')
								#cMess = re.sub(underline, '', commit.msg)
								#commitMessage = formatString(commit.msg)
								...
								while len(pullData) > 0 and int(pullData[0]) == -1 and pullSeen == False:
									print('trying to pull')
									print(myCounter)
									myCounter += 1
									pullData = pullSearch(key, commit.hash)
								if pullSeen == False:
									print(pullData)
								pullNum = -1
								if not len(pullData) == 0 and pullSeen == False:
								...
								if pullSeen == False:
									...
									for line in word_list:
										...
										pullString2 += '\n'
								if pullSeen == False:
									listToWrite.append(pullString2)
									pullingString = pullString2
									pullSeen = True
								else:
									listToWrite.append(pullingString)

								issueNums = []
								issueNumListTentative = parseMessage(key, commit.msg)
								for num in issueNumListTentative:
									if not num in issueNums and not num == pullNum:
										issueNums.append(num)
										nextNumListTentative = parseMessage(key, pullString)
								...
								if issueSeen == False:
									listToWrite.append(issueUlt)
									issuingString = issueUlt
									issueSeen = True
								else:
									listToWrite.append(issuingString)
