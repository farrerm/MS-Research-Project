...
for (i in 1:length(repos)){
  ...
  #str(myDates)
  dataSubSet <- append(dataSubSet, myDates)
  #myDf <- data.frame(dataSubSet$`Annotations added`, myDates)
 # ggplot(myDf, aes(dataSubSet..Annotations.added.)) + stat_lorenz()
  ...
  lineNo <- dataSubSet$`Lines of code`[[length(dataSubSet$`Lines of code`)]]
  #cumAnnots <- vector()
 # sum <- 0
