...
for (i in 1:length(repos)){
  ...
  x <- dataSubSet$`Lines of code`
  y <- dataSubSet$`Annotations added`
  ...
  s2 <- paste(s11, s1, sep = "")
  png(file = s2)
  s3 <- "annotations vs lines of code"
  s4 <- paste(repoName, s3, sep = " ")
  plot(x,y,col = "blue", main = s4, abline(lm(y~x)), xlab = "lines of code", 
       ylab = "no of checkers added")
  dev.off()
