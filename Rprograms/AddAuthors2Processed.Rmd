...
```{r}
write.csv(output, 'ginisPerProj.csv')
```

```{r}
write.csv(checkerData, 'checkDatwAuthors.csv')
```
...
```{r}
output <- cbind(output, nuColumn)

```

```{r}
output <- cbind(output, lineNos)
...
for (i in 1:length(repos)){
  ...
  dataSubSet <- checkerData[checkerData$Project == repoName,]
  print(nrow(dataSubSet))
  #gNum <- gini(dataSubSet$`Annotations added`)
  #nuColumn <- append(nuColumn, gNum)
  myDates <- vector()
  ...
  for (j in 1:length(dataSubSet$`Annotations added`)){
    ...
    if (diff > 90){
      ...
      UniqueAuthors <- unique(allAuthorsForChunk)
      numUniqueAuthors <- length(UniqueAuthors)
      uniqueAuthors <- append(uniqueAuthors, numUniqueAuthors)
      ##calculate gini
     # endLocs <- append(endLocs, dataSubSet$`Lines of code`[[j]])
      
      #reset data vectors
      allAuthorsForChunk <- vector()
      allAnnotsForChunk <- vector()
    }   
    allAuthorsForChunk <- append(allAuthorsForChunk, dataSubSet$authorName[[j]])
...
```{r}
nuFrame <- cbind(nuFrame, uniqueAuthors)
```

```{r}
write.csv(nuFrame, "ginisByMonth.csv")
```
